...
export function SystemArchitectureDisplay({}: SystemArchitectureDisplayProps) {
  const { executeCommand } = useAgentStore();
  const [activeView, setActiveView] = useState<
    "diagram" | "metrics" | "connections" | "dataflow"
  >("diagram");
  ...
  const handleViewChange = (
    view: "diagram" | "metrics" | "connections" | "dataflow",
  ) => {
    setActiveView(view);
    loadData(view);
  };
  ...
  const renderMetricCard = (
    ...
    >
      ...
      <p
        className={`text-2xl font-semibold mt-2 ${color || "text-gray-900 dark:text-white"}`}
      >
        {value}
      </p>
      {subtitle && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-2 border-b border-gray-200 dark:border-gray-800">
        {(["diagram", "metrics", "connections", "dataflow"] as const).map(
          (view) => (
            <button
              key={view}
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-gray-900 dark:text-white">
                          {Math.round(
                            (data.resources.cpu.user +
                              data.resources.cpu.system) /
                              1000,
                          )}
                          ms
                        </span>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-gray-900 dark:text-white">
                          {Math.round(data.resources.memory.rss / 1024 / 1024)}{" "}
                          MB
                        </span>
                      </div>
                    </div>
              ...
              {data.mostActive && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    ({data.mostActive.count} messages)
                  </p>
                </div>
              )}
            </div>
