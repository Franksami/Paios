...
import { AGENT_NUMBERS, AGENT_NAMES, AGENT_DESCRIPTIONS } from '@paios/shared'
...
function getAgentCapabilities(agentNumber: number): string[] {
  const capabilitiesMap: Record<number, string[]> = {
    1: ['revenue-tracking', 'financial-analysis', 'stripe-integration', 'reporting'],
    7: ['web-scraping', 'data-extraction', 'apify-integration', 'automation'],
    8: ['social-media-posting', 'content-scheduling', 'multi-platform', 'analytics'],
    15: ['invoice-generation', 'pdf-creation', 'email-delivery', 'payment-tracking'],
    18: ['research', 'data-gathering', 'report-generation', 'ai-analysis'],
    29: ['content-publishing', 'distribution', 'cross-platform', 'scheduling'],
    30: ['workflow-automation', 'rule-engine', 'cross-agent-communication', 'triggers'],
    31: ['system-visualization', 'architecture-display', 'real-time-updates', 'mermaid-diagrams'],
    32: ['password-management', 'encryption', 'biometric-auth', 'secure-storage'],
...
async function mockAgentExecution(agentNumber: number, action: string, parameters: any) {
  ...
  switch (agentNumber) {
    ...
    default:
      return { message: `Mock response for agent ${agentNumber}`, action, parameters }
  }
}

// Mock implementations for specific agents
function mockBusinessIntelligence(action: string, parameters: any) {
  const mockData = {
    'getRevenue': {
      total: 125000,
      monthly: 12500,
  ...
  return mockData[action as keyof typeof mockData] || { message: 'Business data retrieved' }
}
...
function mockSystemArchitecture(action: string, parameters: any) {
  return {
    diagram: `graph TD
